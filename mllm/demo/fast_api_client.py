...
class QueryModel(BaseModel):
    ...
    boxes_value: list
    boxes_seq: list
    server_url: Optional[str] = 'http://127.0.0.1:12345/shikra'
...
async def query_endpoint(query: QueryModel):
    ...
    if image is not None:
        image = Image.open(image)
    response = query(image, query.text, query.boxes_value, query.boxes_seq, query.server_url)
    _, image = postprocess(response['response'], image=image)
    return {"response": response, "image": str(image)}

@app.get("/example1")
async def example1_endpoint():
